...
* `GET /funds/{fundCode}/analysis?startDate=YYYY-MM-DD&endDate=YYYY-MM-DD[&periods=MA30,MA60,MA120]`
* `POST /funds/refresh`
...
Analysis points contain only trading days, ordered by date. Each point has `date`, `unitNav` (decimal string or null), and `movingAverages`, a map keyed by `MA30`, `MA60`, and `MA120`. Each average has `value` (decimal string or null) and `deviationPercent` (decimal string without `%`, or null); there are no flat MA fields. By default all three periods are returned. Optional `periods` selects one or more distinct periods from these three (for example, `periods=MA30,MA120`). The longest selected period determines the required calendar history: the first displayed trading day must have 29, 59, or 119 earlier A-share trading days within `ARIADNE_MINIMUM_START_DATE`, respectively, or the request returns 400. In particular, omitting `periods` requires MA120 history even for an otherwise valid MA30/MA60 request.
